---
name: deliberative-code-review
description: "与用户协作进行审议式、渐进式代码审查。语言无关。"
---
...
| `todo.md` | 待办事项（主Agent维护） |
| `known_issues.md` | 已知但暂不解决的问题（主Agent维护） |

子agent指令：[reviewer](./reviewer.md)（启动时让agent自行阅读）
...
检查 `{workdir}/scope.md`：
- **不存在**：与用户确认审查目标、依据、重点、背景、排除范围，整理后写入文件
- **存在**：展示当前范围，询问是否调整

### 阶段二：渐进式审查循环

1. 检查 `{workdir}/review.md`，向用户汇报已完成的审查概况（或初始化 N=1）
2. 询问用户本轮审查目标（默认：暂存区全部变更）
3. 用 Agent 工具（`subagent_type: "general-purpose"`）启动审查agent，prompt：

```
请先阅读文件 ./reviewer.md 获取完整工作指令，然后按要求完成以下任务：
...
向用户返回摘要：审查范围、轮次数、问题统计、待办状态。**不**输出审查全文。
