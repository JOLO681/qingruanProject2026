...
description: "通用审议式流程，采用「执行-审查」审议框架完成任意用户任务。语言无关。"
---
...
workdir 应位于工作区的 `deliberations/` 目录下，命名格式为 `deliberations/{YYYYMMDDHHMM}_{简要描述}/`。
...
执行agent完成后，使用 Agent 工具（`subagent_type: "general-purpose"`）启动，prompt 结构：
