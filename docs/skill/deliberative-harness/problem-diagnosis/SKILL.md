---
name: deliberative-problem-diagnosis
description: "采用「诊断-质询」审议框架完成问题定位。语言无关。"
---
...
workdir 应位于工作区的 `diags/` 目录下，命名格式为 `diags/{YYYYMMDDHHMM}_{简要描述}/`。
...
```
请先阅读文件 ./diagnostician.md 获取完整工作指令，然后按要求完成以下任务：
...
诊断agent完成后，使用 Agent 工具（`subagent_type: "general-purpose"`）启动，prompt 结构：

```
请先阅读文件 ./challenger.md 获取完整工作指令，然后按要求完成以下任务：
