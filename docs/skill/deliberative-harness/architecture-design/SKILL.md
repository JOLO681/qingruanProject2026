---
name: deliberative-architecture-design
description: "采用「设计-验证」审议框架完成架构级OOD设计。语言无关，适用于任何编程语言。"
---

## 框架概述

本技能实现一个「设计-验证」审议循环：两个子agent以文件为唯一沟通媒介，交替运行，审议迭代，直至就OOD设计方案达成一致。主Agent仅负责编排，禁止读取子Agent产出文件，仅凭返回状态标记路由。
...
workdir 应位于工作区的 `designs-oo/` 目录下，命名格式为 `designs-oo/{YYYYMMDDHHMM}_{简要描述}/`。
...
使用 Agent 工具（`subagent_type: "general-purpose"`）启动，prompt 结构：
...
设计agent完成后，使用 Agent 工具（`subagent_type: "general-purpose"`）启动，prompt 结构：
