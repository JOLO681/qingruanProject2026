---
name: deliberative-redeliberation
description: "再审议框架 SKILL。只有用户明确要求使用再审议框架时，才使用此 SKILL。使用此 SKILL 时，不需要再加载其他审议式 SKILL，但可以加载准备 SKILL。"
---
...
| 模式文件 | 外部技能 | 引用文件 |
|---------|---------|---------|
| component-a-ood.md | architecture-design | designer.md、verifier.md |
| component-a-execution.md | deliberative-execution | executor.md、reviewer.md |
| component-a-diagnosis.md | problem-diagnosis | diagnostician.md、challenger.md |
| component-a-requirement.md | requirement-design | designer.md、verifier.md |
| component-a-technical.md | technical-design | designer.md、verifier.md |
...
- 校验外部文件依赖：根据当前 `mode` 检查对应模式文件所引用的外部技能指令文件是否存在（如 mode=ood 时检查 `../architecture-design/designer.md` 和 `verifier.md`）。若任一文件不存在，直接向用户报错并终止
- 初始化迭代计数器 `M = 1`
...
使用 Agent 工具（`subagent_type: "general-purpose"`）启动 parser agent，prompt 结构：

```
请先阅读文件 ./parser.md 获取完整工作指令，然后按要求完成以下任务：
...
主Agent阅读 `./component-a-{mode}.md`，扮演该技能的编排角色，逐环节启动子Agent执行工作。关键要点：
...
主Agent阅读 `./component-b.md`，扮演其编排角色。
...
使用 Agent 工具（`subagent_type: "general-purpose"`）启动 judge agent，prompt 结构：

```
请先阅读文件 ./judge.md 获取完整工作指令，然后按要求完成以下任务：
...
使用 Agent 工具（`subagent_type: "general-purpose"`）启动 history-extractor agent，prompt 结构：

```
请先阅读文件 ./history-extractor.md 获取完整工作指令，然后按要求完成以下任务：
