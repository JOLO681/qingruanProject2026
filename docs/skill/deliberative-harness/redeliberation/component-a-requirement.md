...
复用 requirement-design 的子Agent指令文件，启动子Agent时让其自行阅读：

- **需求澄清agent指令**：`../requirement-design/designer.md`
- **需求验证agent指令**：`../requirement-design/verifier.md`
...
```
请先阅读文件 ../requirement-design/designer.md 获取完整工作指令，然后按要求完成以下任务：
...
{若 edit_mode 且 N=1，追加：【首要指令】你的首要任务是修改输出文件中的已有内容，而非从零创建。}
...
```
请先阅读文件 ../requirement-design/verifier.md 获取完整工作指令，然后按要求完成以下任务：
