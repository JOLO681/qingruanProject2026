...
```
页面加载 -> 初始化模块:
  1. Hash路由管理器: 注册hashchange监听, 初始路由解析
  2. Tab栏控制器: 事件委托监听5个Tab按钮点击
  3. postMessage消息总线: 注册message事件监听器(origin校验)
  4. JWT拦截器: 包装全局fetch, 请求拦截(附加Authorization头), 响应拦截(401处理)
  5. FAB按钮: 注册点击事件 -> 唤起AI助手对话弹窗(主框架层渲染, 遮罩覆盖)

路由变更(hashchange触发):
  -> routeGuard(hashPath) 路由守卫检查
    -> 放行: 更新iframe src -> 更新Tab active状态 -> 发送TAB_SWITCH消息至iframe
    -> 拦截(未登录): 跳转#/login?redirect=原路径
    -> 拦截(需改密): 跳转#/change-password
    -> 拦截(权限不足): 跳转#/home

postMessage消息路由(message事件触发):
  -> 校验event.origin(仅同源)
  -> 解析JSON payload
  -> 按type分发:
    AUTH_SYNC: 广播至所有iframe(登录态同步)
    NAVIGATE: 切换Tab+传递params给目标iframe
    DATA_TRANSFER: 缓存至sessionStorage(transfer_data) -> 转发至目标iframe
    TOKEN_EXPIRED: 广播登出 -> 清除Token -> 跳转登录
    LOADING: 更新主框架加载指示器
    HISTORY_BACK: 触发生命周期回退

JWT拦截器(fetch包装):
  -> 请求拦截: 从localStorage读取Token -> 自动附加Authorization: Bearer <token>
  -> 响应拦截: 检查status=401且code=AUTH_REQUIRED
    -> 清除localStorage Token
    -> postMessage广播AUTH_SYNC(token=null)
    -> 展示非阻断Toast("登录已过期，请重新登录")
    -> 提供"重新登录"按钮(跳转#/login?redirect=当前路径)
```

#### profile.html 流程图:

```
页面加载 -> 从localStorage读取Token -> 解析JWT获取user_id/role
  -> GET /api/user/profile -> 渲染用户信息(头像/用户名/注册时间)
  -> 根据role渲染功能入口菜单:
    role=admin: 显示"智能管理"入口(跳转#/admin)
    role=user: 隐藏管理入口
  -> 始终显示: 风险预测入口、打卡记录入口、健康建议入口

修改个人信息:
  -> 点击头像区域 -> 触发隐藏file input -> 选择文件
    -> 前端校验(类型JPEG/PNG/WebP, 大小≤2MB)
    -> POST /api/upload/avatar (multipart/form-data) -> 获取返回URL
    -> PUT /api/user/profile {avatar: url} -> 更新头像显示
  -> 修改用户名 -> PUT /api/user/profile {username: newName}
    -> 校验唯一性 -> 成功刷新显示

退出登录:
  -> 点击"退出登录" -> SweetAlert2确认弹窗
  -> 确认: clearToken() -> postMessage(AUTH_SYNC, {token:null}) -> 跳转#/home
```

#### punch.html 流程图:

```
页面加载 -> 从URL参数或sessionStorage读取筛选条件(默认近30天)
  -> 并行请求:
    GET /api/punch/list?startDate=xxx&endDate=xxx&punch_type=xxx&page=1
    GET /api/punch/analysis (获取打卡分析统计)
  -> 缓存未命中或数据过期: 展示加载态
  -> 成功: 渲染3个区域
    1. 日期筛选器: 两个date input(start/end) + 类型筛选chip按钮组
    2. AI分析统计卡片行: 饮食完成率/运动完成率/总打卡次数(3列)
    3. 打卡记录列表: 每条含类型标签/方案标题/时间/状态/备注

日期筛选变更:
  -> 用户修改startDate或endDate -> 重新请求列表+分析API -> 更新渲染

打卡类型筛选:
  -> 用户点击chip按钮(饮食/运动/全部) -> 更新punch_type参数 -> 重新请求

分页加载更多:
  -> 滚动到底部检测 -> page++ -> GET /api/punch/list?page=N
  -> 追加渲染到列表末尾(无限滚动模式)

查看打卡分析:
  -> 分析数据由GET /api/punch/analysis返回(Dify工作流生成)
  -> 展示: 完成率环形图、近7天趋势柱状图、依从性评语文本、改进建议列表
  -> 分析数据不足7天时标注"数据量提示"
```
...
```
页面加载 -> GET /api/assistant/advice?page=1&pageSize=20
  -> 数据为空: 展示空状态("还没有健康建议，去AI助手对话中获取吧")
  -> 有数据: 渲染可展开的建议卡片列表

建议卡片交互:
  -> 初始状态: 折叠(仅显示标题 + 标签行 + 创建时间)
  -> 点击标题行: 展开卡片 -> 旋转箭头动画 -> 显示完整正文(marked.js渲染Markdown)
  -> 再次点击或点击其他卡片: 收起当前(手风琴模式, 仅一个展开)
...
```
页面加载 -> 解析URL参数(?view=logs则显示日志视图)
  -> 默认视图: 对话视图(类医师对话布局)
    -> 从localStorage读取admin_conversation_id
    -> 渲染对话界面(管理图标 + 欢迎语 + 输入框)
    -> 顶部显示"操作日志"入口按钮(切换至日志视图)
  -> 日志视图: GET /api/admin/logs?page=1 -> 渲染操作日志表格列表

管理员发送自然语言指令:
  -> 构造消息气泡(用户侧) -> 添加到DOM
  -> fetch POST /api/admin/chat (SSE流, 携带JWT)
    -> 读取ReadableStream -> 逐块解析SSE事件
    -> event=message: 追加/更新AI回复气泡(SQL执行过程+结果展示)
    -> event=message_end: 保存conversation_id到localStorage
    -> event=error: 展示警告气泡 + 重试按钮

视图切换:
  -> 点击"操作日志"按钮: 隐藏对话视图, 显示日志列表视图
  -> GET /api/admin/logs?page=1 -> 渲染操作日志(操作类型/时间/内容/结果)
  -> 分页: 底部页码或滚动加载
  -> 点击"返回对话": 隐藏日志视图, 显示对话视图
```
...
```
页面加载 -> 检查是否已有有效Token
  -> 有Token: 解析redirect参数 -> 跳转回目标路径(或默认#/home)
  -> 无Token: 渲染登录表单(默认视图)

登录/注册视图切换:
  -> Tab按钮切换(登录/注册)
  -> 登录视图: 用户名 + 密码 + 登录按钮 + "没有账号? 去注册"链接
  -> 注册视图: 用户名 + 密码 + 确认密码 + 注册按钮 + "已有账号? 去登录"链接

注册表单实时校验:
  -> 用户名: 输入时检查长度(3-50字符) -> 失焦时检查唯一性(GET检查)
  -> 密码: 输入时检查长度(≥8位) + 格式(含字母和数字)
  -> 确认密码: 输入时比对与密码是否一致
  -> 校验失败: 字段下方红色错误文本, 提交按钮可点击(点击后焦点跳转首个错误字段)

登录提交:
  -> POST /api/auth/login {username, password}
  -> 成功: setToken(token) -> postMessage(AUTH_SYNC, {token, role})
    -> 检查must_change_password标志: 是 -> 跳转#/change-password; 否 -> 跳转redirect路径
  -> 失败(401): 展示"用户名或密码错误"提示
  -> 网络错误: 展示"网络连接失败，请检查网络"提示

注册提交:
  -> POST /api/auth/register {username, password}
  -> 成功(201): SweetAlert2提示"注册成功" -> 自动切换至登录视图并预填用户名
  -> 失败(409): 展示"用户名已存在"提示
  -> 失败(422): 展示具体校验错误信息
```
...
```
页面加载 -> 从localStorage读取must_change_password标志
  -> 非'true': 跳转#/home(非强制改密场景不应访问此页)
  -> 为'true': 渲染密码修改表单(锁图标 + 提示文案 + 新密码/确认密码 + 提交按钮)
  -> 页面不可通过Tab栏或返回按钮绕过(路由守卫拦截)

表单实时校验:
  -> 新密码: 输入时检查长度(≥8位) + 格式(含字母和数字)
  -> 确认密码: 输入时比对与新密码是否一致

提交:
  -> PUT /api/user/password {new_password: xxx} (首次修改不传old_password, 后端根据password_changed='0'判断)
  -> 成功: 清除localStorage的must_change_password标志 -> SweetAlert2提示"密码修改成功"
    -> postMessage(AUTH_SYNC广播) -> 跳转#/admin(管理员默认页)
  -> 失败: 展示错误提示 + 允许重试
```
